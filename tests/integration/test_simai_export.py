...
def test_all_pipeline_modes_export_valid_single_channel_benchmarks() -> None:
    ...
    for mode in MODES:
        ...
        case = to_benchmark(built, f"synthetic_{mode}", category="random")
...
def test_topology_export_records_fixed_route_resources() -> None:
    ...
    case = to_benchmark(
        built, "synthetic_routes", topology_path=topology,
        category="random", projection_relation="route_frozen_projection",
    )
