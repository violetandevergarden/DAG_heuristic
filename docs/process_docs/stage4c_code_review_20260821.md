...
本审查依据 `docs/plan_docs/stage4_LLM_search.md` 和 `docs/plan_docs/stage4c_conflict_graph_packing.md`，检查当前固定多资源冲突图与兼容集合选择实现。用户任务中的“stage4a 代码”按上下文解释为 Stage 4c 代码。按 2026-08-22 更新后的实验口径，Stage 3 旧 benchmark 是主要实验输入，100--1000 节点小图用于冻结验证，Stage 4a 只提供少量 real-derived 迁移样例和 1--2 个中大图规模检查。`docs/old_docs/stage4研究` 中的旧审查用于定位历史问题，不作为当前结论依据。
...
现有六个 motif 覆盖团、空冲突图、宽通信与窄通信组合、路径、一次交换和多资源热点等部分机制，并能用 Exact 核对最终 makespan。这些样例适合作为回归材料，但不能替代 Stage 3 主实验和 100--1000 节点冻结验证。
...
runner 仍直接按 Stage 4a manifest 中 `scenario == "muti_channel"` 的顺序取前 N 个样例，没有先运行 Stage 3 固定多资源 benchmark，也没有建立 100--1000 节点冻结验证集和 1--2 个中大图清单。对少量 Stage 4a 派生样例仍需检查转换状态、资源映射和竞争证据，但不再要求全量真实 corpus 承担总体收益验证。

### 4.9 100--1000 节点验证和有限中大图缺少统一硬预算

当前分支直接完整运行 `schedule_pack` 和 `schedule_bounded_packing`，没有墙钟、内存或任务规模保护。新的第二层要求 100--1000 节点样例在统一预算内完整回放，第三层只允许预选 1--2 个中大图逐例检查；现有 runner 两者都无法可靠执行或如实生成 timeout/fallback 行。
...
- Stage 3 主实验、100--1000 节点冻结验证，以及 1--2 个中大图的逐例成本与内存报告。
...
- 复杂 packing 已从 Stage 3 benchmark 稳定迁移到 100--1000 节点小图；
- 当前 runner 已建立公平、分层、可复现的 Stage 4c 三层对照；
- Stage 4c 已完成，或其组件可直接进入 Stage 4g。

## 8. 最终结论

现有 Stage 4c 代码是一个可继续修正的研究原型，不是已验收算法。最优先工作不是扩大实验，而是修正候选选择契约、流式有界枚举和共享预算记账；随后先在 Stage 3 benchmark 完成主筛选和消融，再在 100--1000 节点冻结集检查迁移，最后只用 1--2 个中大图逐例检查规模与成本。旧审查中的核心风险截至本次检查仍未关闭。
