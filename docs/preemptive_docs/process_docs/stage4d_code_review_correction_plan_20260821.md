...
> 2026-08-22 口径更新：实验按 Stage 1--3 主筛选、100--1000 节点冻结验证、1--2 个中大图规模检查推进，不再要求覆盖完整真实 LLM corpus。

## 1. 修正目标

本方案针对 `stage4d_code_review_20260821.md` 的发现。修正顺序是：先使深度、候选和预算配置真实生效；再修正触发特征与缓存；随后在 Stage 1--3 旧 benchmark 上建立决策级标签和主实验；最后用 100--1000 节点小图冻结验证，并只选 1--2 个中大图检查规模。不要在错误信号和软预算上继续调阈值。
...
协作式检查只能作为软预算。100--1000 节点完整验证和 1--2 个中大图检查均增加进程级整图硬超时，由父进程标记 timeout 并保留输入、配置和已完成统计。若单次 completion 无法安全中止，启动前采用保守上限，并明确报告软超时可能超出的实际时长。
...
- 第一层：Stage 1--3 随机、回归和攻击图，作为主筛选与 Exact 数据；
- 第二层：100--1000 节点随机图、攻击图和可追溯 real-derived slice，作为冻结验证；
- 第三层：预先选择的 1--2 个中大图，只作规模与成本压力检查。

Stage 1--3 数据按模板分成开发和 validation，禁止同一模板不同 seed 跨组。阈值只在第一层选择；100--1000 节点验证集不参与调参。来自同一 workload 的相邻 real-derived slice 不得跨开发与验证边界造成泄漏。

少量 real-derived slice 和中大图仍须记录 Stage 4a 来源、转换和竞争证据等级。sampled prefix 未观察到分歧只能标 unknown，不能判为负例；但不再要求全量 72-case corpus 作为正式实验集。
...
不能删除无改善、超时或 fallback 样例后计算收益。若 100--1000 节点冻结验证只有成本下降而质量相同，应将其结论限定为成本控制层；若旧 benchmark 或冻结验证没有稳定净收益、LLM 派生图差异过小，或最坏退化不可接受，则记录为分析工具，不进入 Stage 4g。1--2 个中大图只逐例报告，不计算总体收益。
...
7. D7：冻结 Stage 1--3 开发/validation、100--1000 节点验证集、1--2 个中大图和结果 schema；
8. D8：运行触发、候选、评价和宽深等预算消融；
9. D9：执行 100--1000 节点冻结验证和有限中大图压力检查；
...
| 冻结验证 | 参数在 Stage 1--3 确定，100--1000 节点报告完整率、最坏退化和全部成本 |
| 阶段结论 | 能回答哪些决策值得搜索、能否跨规模迁移及净收益是否值得 |
