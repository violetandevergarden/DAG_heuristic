...
At that event the scheduler may continue it, switch to another eligible
communication, or wait.  Pausing releases the channel and preserves progress.
"""
...
class PreemptiveDAGModel:
    ...
    def legal_actions(self, state: ScheduleState) -> tuple[Action, ...]:
        actions = [Action.run(task_id) for task_id in self.eligible_communications(state)]
...
def assert_preemptive_trace(model: PreemptiveDAGModel, trace: ScheduleTrace) -> None:
    """Check completion, precedence, progress conservation and channel exclusion."""

    if not model.is_finished(trace.final_state):
        raise AssertionError("trace does not complete every task")
    ...
    for task_id, task in task_map.items():
        ...
        if actual != expected:
            raise AssertionError(f"duration mismatch for {task_id}: {actual} != {expected}")
        if task.kind == "compute" and len(spans) != 1:
            raise AssertionError(f"compute {task_id} must have exactly one interval")
        if spans:
            release = max(
                (model.task_runtime(trace.final_state, dep).completed_at or 0 for dep in task.deps),
                default=0,
            )
            if spans[0].start < release:
    ...
    for previous, current in zip(communications, communications[1:]):
        if current.start < previous.end:
            raise AssertionError(f"channel overlap: {previous} and {current}")
