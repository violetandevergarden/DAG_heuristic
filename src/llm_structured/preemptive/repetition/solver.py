...
from core.oracle.pree_single import bounded_event_search
...
def _paired_search(
    ...
    *,
    ...
    def runtime_key(state, index: int) -> tuple[str, int]:
    ...
    def key(state) -> tuple:
        ...
        return tuple((index, runtime_key(state, index)) for index in range(len(model.tasks)))

    def actions(state):
        legal = tuple(model.legal_actions(state))
        return tuple(action for action in legal if not model.eligible_communications(state) or action.kind == "run")

    return bounded_event_search(
        model,
        actions,
        key,
...
def _component_symmetry_search(
    dag: DAG,
    components: Sequence[Sequence[str]],
    *,
    max_states: int = 500_000,
...
exact_oracle_paired = _paired_search
exact_oracle_component_symmetry = _component_symmetry_search
