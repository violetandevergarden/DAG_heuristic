...
def compute(adapter, state, candidates, ledger, config):
    ...
    duration_ratio = max(durations) / max(1, min(durations)) if len(durations) > 1 else 1.0
    duration_spread = (
        (max(durations) - min(durations)) / max(1, max(durations)) if len(durations) > 1 else 0.0
    )
    ...
    for action in candidates:
        ...
        competing = tuple(
            item for item in released if _task_resources(adapter, item) & adapter.resources(action)
        )
        ...
        current_ids = (
            ()
            if action.kind == "wait"
            else ((action.task_id,) if hasattr(action, "task_id") else action.starts)
        )
        current_tail = max((tails.get(item, 0) for item in current_ids), default=0)
        release_metrics[signature] = {
            "released": tuple(sorted(released)),
            "competing": tuple(sorted(competing)),
            "release_tail": release_tail,
            "current_tail": current_tail,
            "advance": transition.after.time - state.time,
        }
    ...
    return {
        ...
        "duration_spread_ratio": duration_spread,
        "top_duration_ratio": duration_ratio,
        "policy_actions": policy_signatures,
        "true_policy_disagreement": any(
            sig != baseline_signature for sig in policy_signatures.values()
        ),
        "critical_release_crossed": bool(
            next_event < candidate_duration
            and base_metric.get("competing")
            and base_metric.get("release_tail", 0) >= base_metric.get("current_tail", 0)
        ),
        "release_gain_spread": gain_spread > 0,
        "release_gain_spread_value": gain_spread,
        "wait_opportunity": bool(
            config.mode == "optional_idle"
            and wait_available
            and next_event < candidate_duration
            and base_metric.get("competing")
            and release_advantage / max(1, base_metric.get("current_tail", 0))
            >= config.release_tail_advantage_ratio
            and next_event / max(1, candidate_duration) <= config.wait_ratio
        ),
        "resource_conflict_spread": resource_spread,
        "wait_available": wait_available,
        "next_event_distance": next_event,
        "selected_action_blocking_duration": candidate_duration,
