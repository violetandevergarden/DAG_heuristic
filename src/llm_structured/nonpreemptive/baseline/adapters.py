...
from .contracts import ActionSignature, DecisionContext, JobIndex, Mode, ReplaySummary
...
class SingleAdapter:
    ...
    def __init__(self, benchmark):
        ...
        self.job_index = JobIndex.from_model(self.model)
        self.job_index = self.job_index.with_weights(_job_weights(benchmark))
...
def _job_weights(benchmark) -> dict[str, float]:
    return {
        str(item["job_id"]): float(item.get("weight", 1.0))
        for item in benchmark.metadata.get("multi_job", ())
        if "job_id" in item
    }
