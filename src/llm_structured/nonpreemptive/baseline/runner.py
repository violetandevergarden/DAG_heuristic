...
from .solver import baseline_action
...
def _job_metrics(benchmark, completions, starts, job_index=None):
    ...
    for record in records:
        members = (
            job_index.job_to_tasks.get(record["job_id"], ())
            if job_index is not None
            else record.get("task_ids", ())
        )
...
def job_metrics_from_state(benchmark, adapter, state):
    ...
    jobs = _job_metrics(benchmark, completions, starts, adapter.job_index)
    if jobs is None:
        return None, None
    ...
    aggregate = {
        ...
        "weighted_completion_time": sum(
            item["completion"] * adapter.job_index.job_weight.get(item["job_id"], 1.0)
            for item in jobs
        ),
