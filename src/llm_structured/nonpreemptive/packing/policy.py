...
def choose_action(model, state, config: PackingConfig) -> PackingDecision:
    ...
    baseline = baseline_action(model, state)
    candidates = list(construct_candidates(model, state, config, budget))
    valid = []
    for candidate in candidates:
        try: model.validate_action(state, candidate.action, config.mode)
        except ValueError: continue
        valid.append(replace(candidate, features=set_features(model, state, candidate.action, budget)))
    selected = baseline
