...
def schedule(benchmark, config: BarrierConfig | None = None) -> BarrierResult:
    ...
    try:
        while not adapter.is_finished(state):
            ...
            baseline = longest_tail_action(adapter, state, config.mode)
            legal = adapter.legal_actions(state, config.mode)
            starts = [item for item in legal if item.kind != "wait"]
            baseline_features = action_features(adapter, graph, state, baseline, config, budget)
            candidates = [(action_features(adapter, graph, state, item, config, budget), item) for item in starts if item != baseline]
            ...
            if challenger is not None and not challenger_features.truncated:
                ...
                if config.method == "barrier_only": selected, reason = challenger, "barrier_only"
                    ...
                    if config.method == "margin": selected, reason = challenger, "margin"
                        ...
                        values = compare(adapter, state, (baseline, challenger), config.mode, budget, config)
