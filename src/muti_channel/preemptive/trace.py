...
from core.trace.common import ForcedIdleInterval, ReplaySummary, ResourceInterval
from muti_channel.preemptive.solver import MultiTrace
...
def assert_multi_resource_trace(
    ...
    expected_idle: list[ForcedIdleInterval] = []
    for decision in trace.decisions:
        ...
        if selected:
            ...
            if eligible:
                raise AssertionError(f"voluntary idle at t={decision.start}, eligible={eligible}")
            active_compute = any(
                span.kind == "compute"
                and span.start <= decision.start < span.end
                for span in trace.intervals
            )
            if not active_compute:
                raise AssertionError(f"forced idle has no future compute at t={decision.start}")
            expected_idle.append(ForcedIdleInterval(decision.start, decision.end))
    if tuple(expected_idle) != trace.forced_idle:
        raise AssertionError("forced-idle intervals do not match empty decisions")
